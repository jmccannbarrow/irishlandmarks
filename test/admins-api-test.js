...
suite('Admin API tests', function() {
    ...
    test('create an admin', async function() {
        ...
        assert(_.some([returnedAdmin], newAdmin), 'returnedUser must be a superset of newAdmin');
    ...
    test('get admins detail', async function() {
        ...
        for (var i = 0; i < admins.length; i++) {
            assert(_.some([allAdmins[i]], admins[i]), 'returnedAdmin must be a superset of newUser');
        }
    });
